...
def _find_aops_binary() -> Path | None:
    ...
    1. AOPS_BIN environment variable
    2. On PATH
    ...
    if env_path := os.environ.get("AOPS_BIN"):
        p = Path(env_path)
        if p.exists() and p.is_file():
            return p

    # 2. On PATH
    if path_binary := shutil.which("aops"):
...
class TaskIndex:
    ...
    - rebuild_fast(): Uses `aops` CLI binary (default when available)
    """
    ...
    def __init__(self, data_root: Path | None = None):
        ...
        self._aops_binary_path: Path | None = _find_aops_binary()
    ...
    def rebuild_fast(self) -> bool:
        """Rebuild index using `aops` CLI binary (nicsuzor/mem).
        ...
        """
        if not self._aops_binary_path:
            logger.debug("aops binary not found, falling back to Python")
            return False
        ...
        # Run `aops graph` with mcp-index format
        output_base = str(self.index_path).removesuffix(".json")
        cmd = [
            str(self._aops_binary_path),
            "graph",
            "-f",
            "mcp-index",
            "-o",
        ...
        try:
            ...
            if not self.load():
                logger.warning("Failed to load index generated by aops graph")
                return False

            logger.info("Rebuilt index using aops graph: %s tasks", len(self._tasks))
            return True

        except subprocess.TimeoutExpired:
            logger.warning("aops graph timed out")
            return False
        except OSError as e:
            logger.warning("Failed to run aops graph: %s", e)
            return False
