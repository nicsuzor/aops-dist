...
model: sonnet
color: cyan
tools: Read, mcp__pkb__search, mcp__pkb__create_task, mcp__pkb__get_task, mcp__pkb__update_task,
  mcp__pkb__list_tasks, Skill
---
...
- `read_file` - For any workflow/rule file you select as relevant but that is not already inline in your input. Read it NOW, before composing output. NEVER reference a workflow file in your output without having read it first.
